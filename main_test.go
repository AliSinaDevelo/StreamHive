...
func TestRun_healthEndpoints(t *testing.T) {
	...
	respStorageIntegrity, err := client.Get(base + "/storage/status")
	require.NoError(t, err)
	defer func() { _ = respStorageIntegrity.Body.Close() }()
	assert.Equal(t, http.StatusOK, respStorageIntegrity.StatusCode)
	var rawStorageIntegrity storageIntegrityStatusResponse
	require.NoError(t, json.NewDecoder(respStorageIntegrity.Body).Decode(&rawStorageIntegrity))
	assert.False(t, rawStorageIntegrity.Enabled)
	assert.True(t, rawStorageIntegrity.Healthy)
	assert.Equal(t, "live", rawStorageIntegrity.ScanConsistency)
	assert.Zero(t, rawStorageIntegrity.Keys)
	assert.Zero(t, rawStorageIntegrity.KeyBytes)
	assert.Zero(t, rawStorageIntegrity.ContentAddressedKeys)
	assert.Zero(t, rawStorageIntegrity.VerifiedKeys)
	assert.Zero(t, rawStorageIntegrity.VerifiedBytes)
	assert.Zero(t, rawStorageIntegrity.OpaqueKeys)
	assert.Zero(t, rawStorageIntegrity.OpaqueBytes)
	assert.Zero(t, rawStorageIntegrity.CorruptKeys)
	assert.Zero(t, rawStorageIntegrity.MissingKeys)
	...
	assert.Contains(t, metrics, "replication_storage_integrity_scans_started")
	assert.Contains(t, metrics, "replication_storage_integrity_scans_completed")
	assert.Contains(t, metrics, "replication_storage_integrity_scans_failed")
	assert.Contains(t, metrics, "replication_storage_integrity_keys_scanned")
	assert.Contains(t, metrics, "replication_storage_integrity_key_bytes_scanned")
	assert.Contains(t, metrics, "replication_storage_integrity_content_addressed_keys")
	assert.Contains(t, metrics, "replication_storage_integrity_verified_keys")
	assert.Contains(t, metrics, "replication_storage_integrity_verified_bytes")
	assert.Contains(t, metrics, "replication_storage_integrity_opaque_keys")
	assert.Contains(t, metrics, "replication_storage_integrity_opaque_bytes")
	assert.Contains(t, metrics, "replication_storage_integrity_corrupt_keys")
	assert.Contains(t, metrics, "replication_storage_integrity_missing_keys")
	assert.Contains(t, metrics, "replication_storage_integrity_scan_duration_ms")
	...
	assert.Zero(t, metrics["replication_storage_integrity_scans_started"])
	assert.Zero(t, metrics["replication_storage_integrity_scans_completed"])
	assert.Zero(t, metrics["replication_storage_integrity_scans_failed"])
	assert.Zero(t, metrics["replication_storage_integrity_keys_scanned"])
	assert.Zero(t, metrics["replication_storage_integrity_key_bytes_scanned"])
	assert.Zero(t, metrics["replication_storage_integrity_verified_keys"])
	assert.Zero(t, metrics["replication_storage_integrity_verified_bytes"])
	assert.Zero(t, metrics["replication_storage_integrity_opaque_keys"])
	assert.Zero(t, metrics["replication_storage_integrity_opaque_bytes"])
	...
	assert.Contains(t, string(body), "streamhive_replication_storage_integrity_scans_started")
	assert.Contains(t, string(body), "streamhive_replication_storage_integrity_scans_completed")
	assert.Contains(t, string(body), "streamhive_replication_storage_integrity_scans_failed")
	assert.Contains(t, string(body), "streamhive_replication_storage_integrity_keys_scanned")
	assert.Contains(t, string(body), "streamhive_replication_storage_integrity_verified_keys")
	assert.Contains(t, string(body), "streamhive_replication_storage_integrity_corrupt_keys")
	assert.Contains(t, string(body), "streamhive_replication_storage_integrity_missing_keys")
...
func TestObserveStorageIntegrityCountsFailedScan(t *testing.T) {
	metrics := &replicationMetrics{}

	_, err := observeStorageIntegrityStatus(context.Background(), storage.NewMemoryStore(), failingInventoryStatusLister{}, metrics)

	assert.Error(t, err)
	assert.Equal(t, uint64(1), metrics.StorageIntegrityScansStarted.Load())
	assert.Zero(t, metrics.StorageIntegrityScansCompleted.Load())
	assert.Equal(t, uint64(1), metrics.StorageIntegrityScansFailed.Load())
	assert.Zero(t, metrics.StorageIntegrityKeysScanned.Load())
	assert.Zero(t, metrics.StorageIntegrityVerifiedKeys.Load())
	assert.Zero(t, metrics.StorageIntegrityCorruptKeys.Load())
	assert.Zero(t, metrics.StorageIntegrityMissingKeys.Load())
}
