...
- **Reconnect:** reconnect retries are bounded by the configured backoff, while the
  ...
  target rather than only the resolved remote address. A disconnect observed while a target loop is
  active becomes one pending retry after loop ownership is released; duplicate callbacks remain
  coalesced. `peer_reconnect_targets`, `peer_reconnect_active`, `peer_reconnect_attempts`,
  `peer_reconnect_failures`, and `peer_reconnect_successes` expose aggregate pressure and recovery
  without target labels. Cancellation during shutdown removes active loops and is not counted as a
  failure.
