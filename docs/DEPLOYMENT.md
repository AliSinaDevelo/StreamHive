...
- **Lifecycle compaction**: `lifecycle_membership_configured`, `lifecycle_membership_members`, and `lifecycle_compaction_blocked` expose the bounded operator fence without peer labels. A missing membership file blocks `-lifecycle-compact`; an explicitly persisted empty membership is valid for a standalone node.
