...
- **P2P shutdown** uses local cancellation plus TCP connection close and has no shutdown message or remote delivery guarantee. `TCPTransport.Drain(ctx)` adds a caller-bounded local staged lifecycle while `Close()` remains the immediate hard stop; see [PEER_DRAIN.md](PEER_DRAIN.md).
