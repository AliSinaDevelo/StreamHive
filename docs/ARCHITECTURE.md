...
- Listener and peer map share a mutex; the accept loop exits when the listener is closed. Current `Close` is an immediate hard stop that waits for the accept loop but not every per-peer goroutine; the bounded staged-drain design and its implementation prerequisites are recorded in [PEER_DRAIN.md](PEER_DRAIN.md).
...
- **P2P shutdown** currently uses local cancellation plus TCP connection close; it has no shutdown message or remote delivery guarantee. The v0.12 drain decision is a future local, bounded, staged lifecycle that preserves the existing wire contract; see [PEER_DRAIN.md](PEER_DRAIN.md).
